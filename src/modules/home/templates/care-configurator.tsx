...
export function CareConfigurator({ minimal = false }: { minimal?: boolean }) {
  ...
  // Wir haben jetzt mehr Steps (1-5)
  const [step, setStep] = useState(1);
  const [selection, setSelection] = useState<{ 
    forWhom?: string; 
    careLevel?: string;
    serviceType?: string; // NEU
    frequency?: string;   // NEU
  }>({});
  ...
  const handleNext = (key: string, value: string) => {
    ...
    // Kurze Verzögerung für UX (Feedback abwarten)
    setTimeout(() => {
      // Wenn wir noch nicht beim letzten Schritt (Ergebnis ist jetzt Step 5) sind
      if (step < 4) {
        setStep(step + 1);
      } else {
        // Nach Step 4 kommt die "Berechnung"
  ...
  return (
    <>
      {/* 1. TEASER SECTION (Nur wenn !minimal) - BLEIBT GLEICH */}
      {!minimal && (
        <section className="relative z-30 mt-0 lg:-mt-24 px-4 md:px-6 pb-20 pointer-events-none">
            <div className="container mx-auto max-w-5xl">
                <div className="bg-white rounded-[2rem] lg:rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 pointer-events-auto overflow-hidden ring-1 ring-slate-100/50">
                    ...
                    {/* Trust Bar (verkürzt für Übersicht, Inhalt bleibt gleich) */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-300">
           ...
           <div className="relative w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 slide-in-from-bottom-8 duration-300">
              ...
              <div className="p-6 md:p-10 overflow-y-auto custom-scrollbar relative">
                ...
                {/* STEP 3: WAS WIRD BENÖTIGT? (NEU) */}
                {step === 3 && (
                    <div className="space-y-8 animate-in slide-in-from-right-4 fade-in">
                        <h3 className="text-2xl font-extrabold text-slate-900 text-center">Wobei wird Hilfe benötigt?</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <button onClick={() => handleNext("serviceType", "Körperpflege/Grundpflege")} className="p-6 rounded-2xl border border-slate-100 bg-white hover:border-[var(--color-primary)] hover:bg-[var(--color-secondary)] hover:shadow-md transition-all flex flex-col items-center text-center gap-3 group cursor-pointer">
                                <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-white group-hover:text-[var(--color-primary)]"><User className="w-6 h-6" /></div>
                                <span className="font-bold text-slate-700 leading-tight">Waschen, Duschen & Anziehen</span>
                            </button>
                            <button onClick={() => handleNext("serviceType", "Medizinische Pflege")} className="p-6 rounded-2xl border border-slate-100 bg-white hover:border-[var(--color-primary)] hover:bg-[var(--color-secondary)] hover:shadow-md transition-all flex flex-col items-center text-center gap-3 group cursor-pointer">
                                <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-white group-hover:text-[var(--color-primary)]"><Stethoscope className="w-6 h-6" /></div>
                                <span className="font-bold text-slate-700 leading-tight">Medikamente, Spritzen & Wunden</span>
                            </button>
                            <button onClick={() => handleNext("serviceType", "Haushalt & Betreuung")} className="p-6 rounded-2xl border border-slate-100 bg-white hover:border-[var(--color-primary)] hover:bg-[var(--color-secondary)] hover:shadow-md transition-all flex flex-col items-center text-center gap-3 group cursor-pointer">
                                <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-white group-hover:text-[var(--color-primary)]"><Coffee className="w-6 h-6" /></div>
                                <span className="font-bold text-slate-700 leading-tight">Haushalt, Einkauf & Gesellschaft</span>
                            </button>
                            <button onClick={() => handleNext("serviceType", "Intensivpflege/24h")} className="p-6 rounded-2xl border border-slate-100 bg-white hover:border-[var(--color-primary)] hover:bg-[var(--color-secondary)] hover:shadow-md transition-all flex flex-col items-center text-center gap-3 group cursor-pointer">
                                <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-white group-hover:text-[var(--color-primary)]"><Activity className="w-6 h-6" /></div>
                                <span className="font-bold text-slate-700 leading-tight">Intensivpflege oder 24h Betreuung</span>
                            </button>
                        </div>
                    </div>
                )}

                {/* STEP 4: HÄUFIGKEIT? (NEU) */}
                {step === 4 && (
                    <div className="space-y-8 animate-in slide-in-from-right-4 fade-in">
                ...
                {isCalculating && (
                    <div className="flex flex-col items-center justify-center py-10 animate-in fade-in zoom-in">
                        ...
                        <h3 className="text-xl font-bold text-slate-900">Auswertung läuft...</h3>
                    </div>
                )}
                ...
                {!isCalculating && step === 5 && (
                    <div className="text-center animate-in zoom-in-95">
                        ...
                        <p className="text-slate-600 mb-8 max-w-md mx-auto">Wir haben Ihre Angaben erfasst. Fordern Sie jetzt Ihre individuelle Beratung an.</p>
                        
                        <Button 
                            onClick={handleFinish}
                            size="lg" 
                            className="w-full h-14 px-10 text-lg bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white rounded-2xl font-bold shadow-xl shadow-[var(--color-primary)]/20 hover:-translate-y-1 transition-all cursor-pointer"
                        >
              ...
              {step > 1 && !isCalculating && step < 5 && (
                  <div className="p-4 border-t border-slate-50 bg-slate-50/50">
                    <button onClick={handleBack} className="text-slate-400 hover:text-[var(--color-primary)] flex items-center gap-2 text-sm font-bold pl-2 transition-colors cursor-pointer">
